...
\begin{abstract}
	We present a new approach for deriving photometric redshifts for deep galaxy and quasar surveys, based on a physical, data driven model of photometric flux measurements with a latent Gaussian Process model of spectral energy distributions (SEDs).
	This approach combines the advantages of machine learning and template fitting methods while overcoming their drawbacks. 
	It constructs a latent space of physical SEDs directly from the training data, and delivers true redshift probability distributions which correctly include effects such as model uncertainties (the inability to constrain some sections of the SEDs with the available training data) and heterogeneity of the training data (eg various sources, missing bands, and residual biases). 
	We consider the latest SDSS data and demonstrate that our method delivers accurate redshift probability distributions, including for high redshift and faint galaxies, even when the training data is incomplete and heterogeneous.
	In particular, the method reduces dependence on spectroscopic training data: It does not require that all kinds of galaxies be represented in the training data, nor that the galaxies at higher redshifts than any training spectrum be identical in SED to those in the training data.
	Finally, we show how recalculation and storage of redshift probability distributions are computationally trivial in our approach, and we discuss the prospects of overcoming the challenges offered by future surveys such as LSST.
	Afvatn5ages: better model, physics and machine learning ideas, can use much more data, cab work in regimes outside of training
...
\paragraph{Underlying SED model} 
We implicitly model SED of galaxies.

\paragraph{Physics of redshift} 
Effect of redshift is known, and also luminosity distance, and magnitude system.

\paragraph{Fluxes}
No significant magnitude biases. If multiple data sets and other bands are used, then magnitudes should be of sufficient quality. If not, training on the target data set with the same magnitudes should absorb some of these biases.

\paragraph{Representativeness of the training}
 Representativeness of the training. But possibility to use much richer data sets and even many band, deep surveys.
...
\paragraph{Physical interpolation} 
...
Not add actual spectra because of magnitudes, and also assumptions needed anyways to extrapolate and redshift them. Instead, use photometry only.

Fluxes can go negative, but this is not really a problem since this will happen in regimes where the SED is not contrained.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Extensions}

\paragraph{Other extragalactic tracers}
Quasars

\paragraph{Classification}
Star-quasar and star-galaxy separation

\paragraph{Mapping to physical models}
...
\eqn{
	\alpha(t,\lambda) = \left\{\begin{array}{ll}
		\alpha_<(t) = 0.0018\ (1-t^{0.56} + t^{9.57}) &\mathrm{if}\quad \lambda \leq \lambda^\mathrm{ref}\\
		\alpha_>(t) = 0.00035\ (1-t^{4.3}) - 0.00022\ t^{0.46}  &\mathrm{if}\quad \lambda > \lambda^\mathrm{ref}\\
	\end{array}\right.
}
...
\equ{
	\mathrm{SED}(\lambda_\mathrm{obs}, z, t, \ell) = \frac{(1+z) \ \ell}{4\pi D^2(z)} \left( \exp\Bigl(-\alpha(t,\frac{\lambda_\mathrm{obs}}{1+z})\ (\frac{\lambda_\mathrm{obs}}{1+z}-\lambda^\mathrm{ref})\Bigr) + R_\nu\Bigl(\frac{\lambda_\mathrm{obs}}{1+z},t\Bigr) \right)
}
...
\eqn{
	f(b, z, t, \ell) &=&\frac{ \int_0^\infty \mathrm{SED}(\lambda_\mathrm{obs}, z, t, \ell) \ W_b(\lambda_\mathrm{obs}) \frac{\d\lambda_\mathrm{obs}}{\lambda_\mathrm{obs}} }{ \int_0^\infty g^\mathrm{AB}\ W_b(\lambda_\mathrm{obs}) \frac{\d\lambda_\mathrm{obs}}{\lambda_\mathrm{obs}}  } \\ 
	&=& \frac{1+z}{4\pi D^2(z)} \frac{\ell}{g^\mathrm{AB} C_b} \int_0^\infty\frac{\d\lambda_\mathrm{obs}}{\lambda_\mathrm{obs}} \left( \exp\Bigl(-\alpha(t,\frac{\lambda_\mathrm{obs}}{1+z})\ (\frac{\lambda_\mathrm{obs}}{1+z}-\lambda^\mathrm{ref})\Bigr) \ + \ R_\nu\Bigl(\frac{\lambda_\mathrm{obs}}{1+z},t\Bigr) \right) \ W_b(\lambda_\mathrm{obs})   \\ 
	&=& \frac{(1+z)^2 \ \ell}{4\pi D^2(z) g^\mathrm{AB} C_b}  \int_0^\infty{\d\lambda_\mathrm{em}} \left( \exp\bigl(-\alpha(t,\lambda_\mathrm{em})\ \left(\lambda_\mathrm{em}-\lambda^\mathrm{ref}(1+z)\right)\bigr) \ + \ R_\nu\bigl({\lambda_\mathrm{em}},t\bigr)  \right) \ V_b\bigl((1+z)\lambda_\mathrm{em}\bigr)
}
...
\eqn{
	f  \sim \mathcal{GP}\Bigl( m(b,z,\ell,t), \ k(b,b',z,z',\ell,\ell',t,t') \Bigr)
}
It is straightforward to show that the mean function is
\eqn{
	m(b,z,\ell,t) = \frac{(1+z)^2\ \ell}{4\pi D^2(z)g^\mathrm{AB} C_b}  \int_0^\infty {\d\lambda}  \exp\Bigl(-\alpha(t,\lambda)\ \left(\lambda-\lambda^\mathrm{ref}\right)\Bigr) \ V_b\Bigl((1+z)\lambda\Bigr)
}
and does not depend on type or photometric band, while the covariance function or kernel is
\eqn{
	k(b,b',z,z',\ell,\ell',t,t') &=& \left( \frac{ (1+z)(1+z') }{4\pi D(z) D(z') g^\mathrm{AB}} \right)^2  \frac{\ell\ell'}{C_bC_{b'}} \int_0^\infty \ k^\lambda\Bigl(t,t',\lambda,\lambda'\Bigr) \ V_b\Bigl((1+z)\lambda\Bigr) \ V_{b'}\Bigl((1+z')\lambda'\Bigr) \ \d\lambda\d\lambda' 
}
...
\eqn{
	p(\hat{\vec{y}}, \bm{\theta}, \bm{\alpha}, \vec{t}, \bm{\ell} | \vec{z}, \vec{b}, \vec{y}, \hat{\mat{x}})	= \frac{p(\hat{\vec{y}}|\vec{y},\mat{X},\hat{\mat{X}},\bm{\alpha}) \ p(\vec{y}|\mat{X},\bm{\alpha}) \ p(\bm{\ell},\vec{t}|\vec{z},\bm{\theta}) \ p(\bm{\theta})\ p(\bm{\alpha})}{p(\vec{y}|\vec{z},\vec{b})}
}
...
\eqn{
	p(z^*,t^*,\ell^* | \{ b_i^*, y_i^* \}, \vec{y}, \vec{z}, \vec{b}, \hat{\mat{X}}) = \int \d\hat{\vec{y}}\int\d\bm{\alpha}\int\d\bm{\theta} \ p(z^*,t^*,\ell^* | \{ b_i^*, y_i^* \}, \vec{y}, \vec{z}, \vec{b}, \hat{\vec{Y}}, \hat{\mat{X}}, \bm{\alpha}, \bm{\theta}) \ p( \hat{\vec{Y}}, \bm{\alpha}, \bm{\theta} |  \vec{y}, \vec{z}, \vec{b}, \hat{\mat{X}})
}
...
\eqn{
	p(z^*,t^*,\ell^* | \{ b_i^*, y_i^* \}, \vec{y}, \vec{z}, \vec{b}, \hat{\vec{y}}, \hat{\mat{X}}, \bm{\alpha}, \bm{\theta}) &\approx& p(z^*,t^*,\ell^* | \{ b_i^*, y_i^* \},\hat{\vec{y}}, \hat{\mat{X}}, \bm{\alpha}, \bm{\theta})
}
\bl{Give more details!}

This can be simplified as
\eqn{
	p(z^*,t^*,\ell^* | \{ b_i^*, y_i^* \},\hat{\vec{y}}, \hat{\mat{X}}, \bm{\alpha}, \bm{\theta}) &=& \frac{p( \{y_i^*\} | \{ b_i^*\}, z^*,t^*,\ell^*,\hat{\vec{y}}, \hat{\mat{X}}, \bm{\alpha}) \ p(z^*,t^*,\ell^* | \bm{\theta}) }{p( \{y_i^*\} | \{ b_i^*\}, \hat{\vec{y}}, \hat{\mat{X}}, \bm{\alpha}, \bm{\theta}) } \\
	&\propto& p(z^*,t^*,\ell^*| \bm{\theta}) \ \prod_i p( y_i^* | b_i^*, z^*,t^*,\ell^*,\hat{\vec{y}}, \hat{\mat{X}}, \bm{\alpha}) 
}
...
\equ{
	R_\nu(\lambda,t) = C(\lambda,t) + \sum_{l} A(\lambda,t) \gaussian(\lambda, \lambda_l, \sigma_l)
}
...
\eqn{
	C(\lambda,t) \sim \GP\left(0, k_C(\lambda, \lambda')k_t(t,t')\right)\\
	A(\lambda,t) \sim \GP\left(0, k_A(\lambda, \lambda')k_t(t,t')\right)
}
If one further assumes that $C$ and $A$ are uncorrelated, then 
\eqn{
	f(\lambda,t) \sim \GP\left(0, K_L(\lambda, \lambda')k^t(t,t')\right)
}
with 
\eqn{
	k^f(\lambda, \lambda') = K_C(\lambda, \lambda') + k^A(\lambda, \lambda')\sum_{l} \gaussian(\lambda, \lambda_l, \sigma_l) \gaussian(\lambda', \lambda_l, \sigma_l)
	}
...
\eqn{
	...
	k^t(t,t') &=& \ \ \quad  \mathcal{N}\left(t, t', {\alpha}_t\right) \\
}
...
In this case, one can derive an exact expression for the Gaussian Process mean function kernel in flux space, by calculating the definiting integral in two sections $[0, \lambda^\mathrm{ref}]$ and $[\lambda^\mathrm{ref}, \infty]$ with the $\alpha_>(t)$ and $\alpha_>(t)$ coefficients, respectively. The result is
\eqn{
	m(b,z,\ell,t) &=& \frac{\ell\  (1+z)^2}{4\pi D^2(z)g^\mathrm{AB} C_b} \sum_{i} a_i  \int_0^\infty{\d\lambda} \exp\Bigl(-\alpha(t,\lambda)\ \left(\lambda-\lambda^\mathrm{ref}\right)\Bigr) \ \mathcal{N}\Bigl((1+z)\lambda,\mu_i,\sigma_i\Bigr) \\
	&=& \frac{\ell\  (1+z)^2}{4\pi D^2(z)g^\mathrm{AB} C_b} \sum_{i} a_i  \Bigl(I_i(\alpha_>(t), z, \infty) - I_i(\alpha_>(t), z, \lambda^\mathrm{ref}) + I_i(\alpha_<(t), z, \lambda^\mathrm{ref}) - I_i(\alpha_<(t), z, 0) \Bigl)\nonumber
}
with the indefinite integral 
\eqn{
	I_i(\alpha, z, \lambda') &=& \int_0^{\lambda'}{\d\lambda} \exp\Bigl(-\alpha\left(\lambda-\lambda^\mathrm{ref}\right)\Bigr) \ \mathcal{N}\Bigl((1+z)\lambda,\mu_i,\sigma_i\Bigr) 
	\\ &=& \frac{\sigma_i}{1+z}\sqrt{\frac{\pi}{2}} \ \mathrm{erf}\left( \frac{\alpha\sigma_i^2 - \mu_i(1+z) + \lambda(1+z)^2}{\sqrt{2}\sigma_i(1+z)}\right) \ \exp\left( \frac{\alpha}{2(1+z)^2}\Bigl( \alpha\sigma_i^2 - \frac{\mu_i}{1+z} + \lambda^\mathrm{ref}\Bigr)\right)
}
The kernel is
\eqn{
	k(b,b',z,z',\ell,\ell',t,t') &=& \left( \frac{ (1+z)(1+z') }{4\pi D(z) D(z') g^\mathrm{AB}} \right)^2 \frac{\ell \ell'}{C_bC_{b'}} \nonumber \\
...
\eqn{
	p(l | t,\bm{\theta}) = \frac{1}{\Gamma(\alpha(t,\bm{\theta})+1)}\frac{1}{\ell^*} \left(\frac{l}{\ell^*}\right)^{\alpha(t,\bm{\theta})} \exp \left(\frac{l}{\ell^*}\right)
}
...
\eqn{
	p(\ell,t|z,\bm{\theta}) =  p(\ell|t,\bm{\theta}) p(t|\bm{\theta}) .
}
...
\eqn{
	\mathcal{U}(\hat{\vec{y}}, \bm{\theta}, \bm{\alpha}, \vec{t}, \bm{\ell}) &=& -\ \ln p(\hat{\vec{y}}, \bm{\theta}, \bm{\alpha}, \vec{t}, \bm{\ell} | \vec{z}, \vec{b}, \vec{y}, \hat{\mat{x}}) \\
		&=& - \ \ln p(\hat{\vec{y}}|\vec{y},\mat{X},\hat{\mat{X}},\bm{\alpha}) \ -\ \ln p(\vec{y}|\mat{X},\bm{\alpha}) \ -\ \ln p(\bm{\ell},\vec{t}|\vec{z},\bm{\theta}) \\ && -\ \ln p(\bm{\theta},\bm{\alpha}) \ + \ \ln p(\vec{y}|\vec{z},\vec{b}) 
}
