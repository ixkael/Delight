...
\equ{
	\mathrm{SED}(\lambda_\mathrm{obs}, z, t, \ell) = \frac{1+z}{4\pi D^2(z)} \left(\ell\ \exp\Bigl(-\alpha(t)\ (\lambda_\mathrm{obs}-\lambda^\mathrm{ref})\Bigr) + R_\nu\Bigl(\frac{\lambda_\mathrm{obs}}{1+z},t\Bigr) \right)
}
...
\eqn{
	...
	&=& \frac{1+z}{4\pi D^2(z)} \frac{1}{g^\mathrm{AB} C_b} \int\frac{\d\lambda_\mathrm{obs}}{\lambda_\mathrm{obs}} \left( \ \ell \exp\Bigl(-\alpha(t)\ (\lambda_\mathrm{obs}-\lambda^\mathrm{ref})\Bigr) \ + \ R_\nu\Bigl(\frac{\lambda_\mathrm{obs}}{1+z},t\Bigr) \ W_b(\lambda_\mathrm{obs}) \right)   \\ 
	&=& \frac{(1+z)^2}{4\pi D^2(z) g^\mathrm{AB} C_b}  \int{\d\lambda_\mathrm{em}} \left( \ \ell \exp\Bigl(-\alpha(t)\ \left((1+z)\lambda_\mathrm{em}-\lambda^\mathrm{ref}\right)\Bigr) \ + \ R_\nu\Bigl({\lambda_\mathrm{em}},t\Bigr) \ V_b\Bigl((1+z)\lambda_\mathrm{em}\Bigr) \right) 
}
...
\eqn{
	f  \sim \mathcal{GP}\Bigl( m(b,z,\ell,t), \ k(b,b',z,z',t,t') \Bigr)
}
It is straightforward to show that the mean function is
\eqn{
	m(b,z,\ell,t) = \frac{(1+z)^2}{4\pi D^2(z)g^\mathrm{AB} C_b}  \int{\d\lambda}  \ \ell \exp\Bigl(-\alpha(t)\ \left((1+z)\lambda-\lambda^\mathrm{ref}\right)\Bigr)
}
...
\eqn{
	m(b,z,\ell,t) = \frac{\ell\  (1+z)}{8\pi D^2(z)g^\mathrm{AB} C_b} \sum_{i} a_i \left( 1 + \mathrm{erf}\ \frac{\mu_i(1+z)-\alpha(t)\sigma_i^2}{\sqrt{2}\sigma_i(1+z)} \ \right) \exp\left( \alpha(t)\Bigl( \frac{\alpha(t)\sigma_i^2}{2(1+z)^2} - \frac{\mu_i}{1+z} + \lambda^\mathrm{ref}\Bigr)\right)
}
The kernel is
\eqn{
	k(b,b',z,z',t,t') &=& \left( \frac{ (1+z)(1+z') }{4\pi D(z) D(z') g^\mathrm{AB}} \right)^2  \nonumber \\
	  \times \ \sum_{i}\sum_{i'} \frac{a_{i} a_{i'}}{C_bC_{b'}}&&\hspace*{-5mm} \left( 2\pi\sigma_{i}\sigma_{i'} V_C\ K_C(z,z',b,b',i,i') + V_L \ \sum_{l}\sum_{\ell'} K_L(z,z',b,b',i,i',\ell,\ell')\right)
}
