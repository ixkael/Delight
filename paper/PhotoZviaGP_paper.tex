...
\equ{
	\mathrm{SED}(\lambda_\mathrm{obs}, z, t, \ell) = \frac{(1+z) \ \ell}{4\pi D^2(z)} \left( \exp\Bigl(-\alpha(t)\ (\frac{\lambda_\mathrm{obs}}{1+z}-\lambda^\mathrm{ref})\Bigr) + R_\nu\Bigl(\frac{\lambda_\mathrm{obs}}{1+z},t\Bigr) \right)
}
...
\eqn{
	f(b, z, t, \ell) &=&\frac{ \int \mathrm{SED}(\lambda_\mathrm{obs}, z, t, \ell) \ W_b(\lambda_\mathrm{obs}) \frac{\d\lambda_\mathrm{obs}}{\lambda_\mathrm{obs}} }{ \int g^\mathrm{AB}\ W_b(\lambda_\mathrm{obs}) \frac{\d\lambda_\mathrm{obs}}{\lambda_\mathrm{obs}}  } \\ 
	&=& \frac{1+z}{4\pi D^2(z)} \frac{\ell}{g^\mathrm{AB} C_b} \int\frac{\d\lambda_\mathrm{obs}}{\lambda_\mathrm{obs}} \left( \exp\Bigl(-\alpha(t)\ (\frac{\lambda_\mathrm{obs}}{1+z}-\lambda^\mathrm{ref})\Bigr) \ + \ R_\nu\Bigl(\frac{\lambda_\mathrm{obs}}{1+z},t\Bigr) \right) \ W_b(\lambda_\mathrm{obs})   \\ 
	&=& \frac{(1+z)^2 \ \ell}{4\pi D^2(z) g^\mathrm{AB} C_b}  \int{\d\lambda_\mathrm{em}} \left( \exp\bigl(-\alpha(t)\ \left(\lambda_\mathrm{em}-\lambda^\mathrm{ref}\right)\bigr) \ + \ R_\nu\bigl({\lambda_\mathrm{em}},t\bigr)  \right) \ V_b\bigl((1+z)\lambda_\mathrm{em}\bigr)
}
...
\eqn{
	m(b,z,\ell,t) = \frac{(1+z)^2\ \ell}{4\pi D^2(z)g^\mathrm{AB} C_b}  \int{\d\lambda}  \exp\Bigl(-\alpha(t)\ \left(\lambda-\lambda^\mathrm{ref}\right)\Bigr) \ V_b\Bigl((1+z)\lambda\Bigr)
}
and does not depend on type or photometric band, while the covariance function or kernel is
\eqn{
	k(b,b',z,z',\ell,\ell',t,t') &=& \left( \frac{ (1+z)(1+z') }{4\pi D(z) D(z') g^\mathrm{AB}} \right)^2  \frac{\ell\ell'}{C_bC_{b'}} \int \ k^\lambda\Bigl(t,t',\lambda,\lambda'\Bigr) \ V_b\Bigl((1+z)\lambda\Bigr) \ V_{b'}\Bigl((1+z')\lambda'\Bigr) \ \d\lambda\d\lambda' 
}
...
\eqn{
	m(b,z,\ell,t) = \frac{\ell\  (1+z)}{4\pi D^2(z)g^\mathrm{AB} C_b} \sum_{i} a_i \sigma_i\frac{\pi}{2}  \left( 1 + \mathrm{erf}\ \frac{\mu_i(1+z)-\alpha(t)\sigma_i^2}{\sqrt{2}\sigma_i(1+z)} \ \right) \exp\left( \alpha(t)\Bigl( \frac{\alpha(t)\sigma_i^2}{2(1+z)^2} - \frac{\mu_i}{1+z} + \lambda^\mathrm{ref}\Bigr)\right)
}
The kernel is
\eqn{
	k(b,b',z,z',\ell,\ell',t,t') &=& \left( \frac{ (1+z)(1+z') }{4\pi D(z) D(z') g^\mathrm{AB}} \right)^2 \frac{\ell \ell'}{C_bC_{b'}} \nonumber \\
	  \times \ \sum_{i}\sum_{i'} a_{i} a_{i'} &&\hspace*{-5mm} \left( 2\pi\sigma_{i}\sigma_{i'} V_C\ K_C(z,z',b,b',i,i') + V_L \ \sum_{l}\sum_{\ell'} K_L(z,z',b,b',i,i',\ell,\ell')\right)
}
...
\eqn{
	K_L(z,z',b,b',i,i',\ell,\ell') \ = \  \mathcal{N}\left( \mu_{i}, \mu_{\ell}(1+z), \sigma_{i}\right) \ \mathcal{N}\left( \mu_{i'}, \mu_{\ell'}(1+z'), \sigma_{i}\right) \ \mathcal{N}\left( \mu_{\ell}, \mu_{\ell'}, {\alpha_L}\right)
}	
