...
def test_schechter_derivatives():
    ...
    for i in range(size):
        ...
        v2 = derivative(f_ell, ell, dx=0.01, n=1, args=(t, ellStar, alpha0, alpha1), order=5)
        ...
        def f_t(t, ell, ellStar, alpha0, alpha1):
            dist2 = Schechter(ellStar, alpha0, alpha1)
            return dist2.lnpdf(ell, t)
        v2 = derivative(f_t, t, dx=0.01, n=1, args=(ell, ellStar, alpha0, alpha1), order=5)
        assert abs(v1/v2-1) < 0.01
        ...
        def f_alpha0(alpha0, t, ell, ellStar, alpha1):
            dist2 = Schechter(ellStar, alpha0, alpha1)
            return dist2.lnpdf(ell, t)
        v2 = derivative(f_alpha0, alpha0, dx=0.01, n=1, args=(t, ell, ellStar, alpha1), order=5)
        assert abs(v1/v2-1) < 0.01
        ...
        def f_alpha1(alpha1, t, ell, ellStar, alpha0):
            dist2 = Schechter(ellStar, alpha0, alpha1)
            return dist2.lnpdf(ell, t)
        v2 = derivative(f_alpha1, alpha1, dx=0.01, n=1, args=(t, ell, ellStar, alpha0), order=5)
        assert abs(v1/v2-1) < 0.01
...
def test_rayleigh_derivatives():
    ...
    for i in range(size):
        ...
        v2 = derivative(f_z, z, dx=0.01, n=1, args=(t, alpha0, alpha1), order=5)
        assert abs(v1/v2-1) < 0.01
        ...
        def f_t(t, z, alpha0, alpha1):
            dist2 = Rayleigh(alpha0, alpha1)
            return dist2.lnpdf(z, t)
        v2 = derivative(f_t, t, dx=0.01, n=1, args=(z, alpha0, alpha1), order=5)
        assert abs(v1/v2-1) < 0.01
        ...
        def f_alpha0(alpha0, t, z, alpha1):
            dist2 = Rayleigh(alpha0, alpha1)
            return dist2.lnpdf(z, t)
        v2 = derivative(f_alpha0, alpha0, dx=0.01, n=1, args=(t, z, alpha1), order=5)
        assert abs(v1/v2-1) < 0.01
        ...
        def f_alpha1(alpha1, t, z, alpha0):
            dist2 = Rayleigh(alpha0, alpha1)
            return dist2.lnpdf(z, t)
        v2 = derivative(f_alpha1, alpha1, dx=0.01, n=1, args=(t, z, alpha0), order=5)
        assert abs(v1/v2-1) < 0.01
