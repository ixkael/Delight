...
class PhotozGP(Model):
    ...
    """
    def __init__(self,
                 redshifts, luminosities, types, unfixed_indices, gp_indices,
        ...
        assert np.min(gp_indices) >= 0 and np.max(gp_indices) < nd
        self.gp_indices = copy(gp_indices)

        self.noisy_fluxes = noisy_fluxes
        self.flux_variances = flux_variances
    ...
    def set_X_inducing(self, X_inducing):
        ...
        if X_inducing is None:
            self.Y_inducing_mean = None
            self.Y_inducing_std = None
        else:
    ...
    def parameters_changed(self):
        ...
        self.Y = ObsAr(
            self.noisy_fluxes[self.gp_indices, :].T.reshape((-1, 1)))
        nd, self.output_dim = self.Y.shape
        self.Y_metadata = {'output_index': np.arange(nd)[:, None]}

        self.likelihood = HeteroscedasticGaussian(self.Y_metadata)
        flux_variances_flat\
            = self.flux_variances[self.gp_indices, :].T.reshape((-1, 1))
        self.likelihood.variance.fix(flux_variances_flat + self.extranoise)

        nd = self.gp_indices.size
        self.X = np.zeros((nd*self.numBandsUsed, 4))
        for i in range(self.numBandsUsed):
            self.X[i*nd:(i+1)*nd, 0] = self.bandsUsed[i]
            self.X[i*nd:(i+1)*nd, 1] = self.redshifts[self.gp_indices, 0]
            self.X[i*nd:(i+1)*nd, 2] = self.luminosities[self.gp_indices, 0]
            self.X[i*nd:(i+1)*nd, 3] = self.types[self.gp_indices, 0]
