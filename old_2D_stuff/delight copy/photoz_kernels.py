...
class Photoz_kernel_1D:
    """
    1D Photoz kernel based on RBF kernel, not using GPy.
    """
    def __init__(self, fcoefs_amp, fcoefs_mu, fcoefs_sig,
                 lines_mu, lines_sig,
    ...
    def predict(self, X, y, yerr, X_pred, stddev=True):
        KXX = self.K(X)
        A = KXX + np.diag(yerr**2.)
        L = scipy.linalg.cholesky(A, lower=True)
        KXXp = self.K(X_pred, X)
        KXpXp = self.K(X_pred)
        alpha = scipy.linalg.cho_solve((L, True), y)
        y_pred = np.dot(KXXp, alpha)
        v = scipy.linalg.cho_solve((L, True), KXXp.T)
        y_pred_fullcov = KXpXp - KXXp.dot(v)
        if stddev:
            return y_pred, np.sqrt(np.diag(y_pred_fullcov))
        else:
